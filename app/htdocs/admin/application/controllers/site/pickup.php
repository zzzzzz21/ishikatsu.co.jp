...
class Pickup extends MY_Controller {
	...
	public function index()
	{
		...
		$list = $this->Pickup_model->get_list();
		$this->assign('list', $list);
		// ワンタイム
		$this->set_unique_hash();
		$this->view('site/pickup.tpl');
	}
	...
	public function complete()
	{
		...
		$this->load->model('Pickup_model');
		$this->Pickup_model->load->database();
		$this->Pickup_model->db->trans_begin();
		...
		foreach ($title as $key => $val)
		{
			...
			$ret = $this->Pickup_model->regist($form);
			// 戻り値から、失敗時には、ロールバックを行う
			$this->complete_fail($ret, '');

			// ファイルアップロード
			$filename_pc = $this->upload_image_array('filename_pc', 'pickup_image_path', date('YmdHis') . '_' . ($key+1), $key);
			...
			{
				$ret = $this->Pickup_model->update_filename_pc($key+1, $filename_pc);
				$this->complete_fail($ret, '');
			}

			// ファイルアップロード
			$filename_sp = $this->upload_image_array('filename_sp', 'pickup_image_path', date('YmdHis') . '_' . ($key+1), $key);
			...
			{
				$ret = $this->Pickup_model->update_filename_sp($key+1, $filename_sp);
				$this->complete_fail($ret, '');
			}
		}

		$this->Pickup_model->db->trans_commit();
	...
	{
		$image_path = $this->config->item('pickup_image_path');
		$image_sync_info = $this->config->item('pickup_image_sync_info');
